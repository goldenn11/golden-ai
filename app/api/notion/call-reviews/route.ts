...
export async function GET() {
  ...
  if (!apiKey || !dbId) {
    return NextResponse.json(
      { error: 'NOTION_API_KEY and NOTION_CALL_REVIEWS_DB_ID must be set' },
      { status: 500 }
    )
  }

  try {
    const results: CallReview[] = []
    let hasMore = true
    ...
    while (hasMore) {
      ...
      for (const page of data.results) {
        ...
        results.push({
          ...
          objection: extractSelect(props['Objection']),
          overcome: extractCheckbox(props['Overcome']),
          contractValue: extractNumber(props['Contract Value']),
          cashCollected: extractNumber(props['Cash Collected']),
          topActions: extractRichText(props['Top Actions']),
          summary: extractRichText(props['Summary']),
          fathomLink: extractUrl(props['Fathom Link']),
    ...
    console.error('Notion call-reviews error:', msg)
